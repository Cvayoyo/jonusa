(function ($) {
    ...
    MultipleSelect.prototype = {
        ...
        events: function () {
            ...
            this.$selectGroups.off("click").on("click", function () {
                var group = $(this).parent().attr("data-group"),
                    ...
                    checked =
                        $children.length !==
                        $children.filter(":checked").length;
        ...
        open: function () {
            ...
            var choise_width =
                this.$choice[0].getBoundingClientRect().width - 2;
            this.$drop.css({ width: choise_width + "px" });

            var choise_top = this.$choice[0].getBoundingClientRect().top;

            if (
                $(window).height() - choise_top - this.$drop.height() - 24 <
                0
            ) {
                this.$drop.css({ top: "" });
                this.$drop.css({
                    bottom: $(window).height() - choise_top + "px",
                });

                if (this.options.filter) {
                    var max_height =
                        $(window).height() -
                        ($(window).height() - choise_top) -
                        32;
                } else {
                    var max_height =
                        $(window).height() - ($(window).height() - choise_top);
                }
        ...
        update: function (isInit) {
            var selects = this.options.displayValues
                    ? this.getSelects()
            ...
            if (sl === 0) {
                ...
                if (sl == 1) {
                    ...
                    $span
                        ...
                        .html(this.options.allSelected);
                }
            } else if (
                this.options.ellipsis &&
                sl > this.options.minimumCountSelected
            ) {
                $span
                    .removeClass("placeholder")
                    .text(
                ...
                $span
                    .removeClass("placeholder")
                    .text(selects.join(this.options.delimiter));
            }
        ...
        getSelects: function (type) {
            var that = this,
                ...
                values = [];
            this.$drop
                .find(sprintf("input[%s]:checked", this.selectItemName))
                .each(function () {
        ...
        setSelects: function (values) {
            ...
            $.each(that.$selectGroups, function (i, val) {
                var group = $(val).parent().attr("data-group"),
                    $children = that.$selectItems.filter(
                        '[data-group="' + group + '"]'
                    );
                $(val).prop(
                    "checked",
        ...
        filter: function () {
            ...
            if (text.length === 0) {
                ...
                this.$selectItems.each(function () {
                    var $parent = $(this).parent();
                    $parent[
                        removeDiacritics($parent.text().toLowerCase()).indexOf(
                ...
                this.$selectGroups.each(function () {
                    ...
                    var group = $parent.attr("data-group"),
                        $items = that.$selectItems.filter(":visible");
                    $parent[
                        $items.filter(sprintf('[data-group="%s"]', group))
                            .length
                            ? "show"
                            : "hide"
                    ]();
