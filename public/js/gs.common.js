...
function getMarkerIcon(imei, speed, status, arrow_color) {
    if (settingsObjectData[imei]["map_icon"] == "arrow") {
        ...
        if (status == false) {
            ...
            if (status == "off") {
                arrow =
                    settingsObjectData[imei]["map_arrows"][
                        "arrow_no_connection"
                    ];
            } else if (status == "m") {
                arrow = settingsObjectData[imei]["map_arrows"]["arrow_moving"];
...
function switchLanguageTracking() {
    ...
    $.ajax({
        type: "POST",
        url: "http://127.0.0.1:50/func/fn_settings",
        headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
            "Content-Type": "application/json",
        },
...
function getNearestMarker(imei, lat, lng) {
    ...
    for (var marker_id in placesMarkerData["markers"]) {
        ...
        var temp = getLengthBetweenCoordinates(
            lat,
            lng,
            marker_lat,
            marker_lng
    ...
    if (sensor != false) {
        ...
        for (i = 0; i < ble_array.length; i += 1) {
            ...
            for (var driver_id in drivers) {
                if (
                    driver_assign_id ==
                    drivers[driver_id].assign_id.toUpperCase()
                ) {
    ...
    if (sensor != false) {
        ...
        for (i = 0; i < ble_array.length; i += 1) {
            ...
            for (var trailer_id in trailers) {
                if (
                    trailer_assign_id ==
                    trailers[trailer_id].assign_id.toUpperCase()
                ) {
