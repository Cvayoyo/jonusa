    ...
    switch (cmd) {
        case "open":
            ...
            if (cpValues["privileges"] == "manager") {
                if (cpValues["obj_days"] == "true") {
                    ...
                    ).value = cpValues["obj_days_dt"];
                } else {
                    document.getElementById(
                        "dialog_object_add_object_expire_dt"
                    ).value = moment().add("years", 1).format("YYYY-MM-DD");
                }
            } else {
                document.getElementById(
                    "dialog_object_add_object_expire_dt"
        ...
        case "add":
            ...
            var model = document.getElementById(
                "dialog_object_add_model"
            ).value;
            var vin = document.getElementById("dialog_object_add_vin").value;
            var plate_number = document.getElementById(
                "dialog_object_add_plate_number"
            ).value;
            var device = document.getElementById(
                "dialog_object_add_device"
            ).value;
            var sim_number = document.getElementById(
                "dialog_object_add_sim_number"
            ).value;
            if (userID == "1") {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result == "OK") {
                        ...
                        $("#cpanel_unused_object_list_grid").trigger(
                            "reloadGrid"
                        );
    ...
    switch (cmd) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    document.getElementById("dialog_object_edit_model").value =
                        result["model"];
                    document.getElementById("dialog_object_edit_vin").value =
                        result["vin"];
                    document.getElementById(
                        "dialog_object_edit_plate_number"
                    ).value = result["plate_number"];
                    document.getElementById("dialog_object_edit_device").value =
                        result["device"];
                    document.getElementById(
                        "dialog_object_edit_sim_number"
                    ).value = result["sim_number"];
                    document.getElementById(
                        "dialog_object_edit_manager_id"
                    ).value = result["manager_id"];
                    $("#dialog_object_edit_manager_id").multipleSelect(
                        "refresh"
                    );
                    ...
                    $(
                        "#dialog_object_edit_users"
                    ).tokenize().options.newElements = true;
                    ...
                    $(
                        "#dialog_object_edit_users"
                    ).tokenize().options.newElements = false;
                },
            });
        ...
        case "save":
            ...
            if (imei != cpValues["edit_object_imei"]) {
                ...
                confirmDialog(
                    la["ARE_YOU_SURE_YOU_WANT_TO_CHANGE_OBJECT_IMEI"],
                    function (response) {
                        if (response) {
                            responseSave();
                        }
                    ...
                    if (response) {
                        ...
                        $.ajax({
                            ...
                            success: function (result) {
                                ...
                                if (result == "OK") {
                                    initStats();
                                    $("#cpanel_object_list_grid").trigger(
                                        "reloadGrid"
                                    );
                                } else if (
                                    result == "ERROR_SYSTEM_OBJECT_LIMIT"
                                ) {
                                    notifyDialog(
                                        la["SYSTEM_OBJECT_LIMIT_IS_REACHED"]
                                    );
                                }
                            },
