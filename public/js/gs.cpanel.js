...
function switchCPManager(manager_id) {
    ...
    $("#cpanel_user_list_grid").setGridParam({
        url:
            "api/cpanel/load_all_user_list_search?cmd=load_user_list&manager_id=" +
            cpValues["manager_id"],
    });
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            document.getElementById("object_list_stats").innerHTML =
                ...
                "/" +
                result["total_objects_online"] +
                ")";

            if (
                document.getElementById("unused_object_list_stats") != undefined
            ) {
                document.getElementById("unused_object_list_stats").innerHTML =
                    "(" + result["total_unused_objects"] + ")";
            }

            if (
                document.getElementById("billing_plan_list_stats") != undefined
            ) {
                document.getElementById("billing_plan_list_stats").innerHTML =
                    "(" + result["total_billing_plans"] + ")";
            }

            document.getElementById(
                "cpanel_manage_server_sms_gateway_total_in_queue"
            ).innerHTML = result["sms_gateway_total_in_queue"];
                            ...
                            success: function (result) {
                                if (result == "OK") {
                                    ...
                                    document.getElementById(
                                        ...
                                        " " +
                                        la[
                                            "PLEASE_CHECK_SERVER_EMAIL_SMTP_SETTINGS"
                                        ];
                                }
                            },
                        });
...
function notifyCheck(what) {
    switch (what) {
        ...
        case "smtp_server":
            if (
                document.getElementById("cpanel_manage_server_email").value ==
                ""
            ) {
                if (cpValues["privileges"] == "super_admin") {
                    confirmDialog(
                        la[
                            "PLEASE_SET_SMTP_SERVER_SETTINGS_IN_MANAGE_SERVER_SECTION"
                        ],
                        function (response) {
                            if (response) {
                                switchCPTab("manage_server");
                                document
                                    .getElementById("manage_server_email_tab")
                                    .click();
                            }
                        }
                    );
    ...
    switch (cmd) {
        ...
        case "save":
            ...
            if (cpValues["set_expiration"] == "users") {
                ...
                confirmDialog(
                    la[
                        "ARE_YOU_SURE_YOU_WANT_TO_SET_EXPIRATION_FOR_SELECTED_ITEMS"
                    ],
                    function (response) {
                        if (response) {
                            var data = {
                        ...
                        if (response) {
                            ...
                            $.ajax({
                                ...
                                success: function (result) {
                                    if (result == "OK") {
                                        $(
                                            "#dialog_user_edit_object_list_grid"
                                        ).trigger("reloadGrid");
